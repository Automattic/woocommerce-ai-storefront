...
- **Your meta description appeared twice when another SEO plugin was active (#669).**
  ...
  - Where the other plugin supplies nothing, your store still publishes its own, so a page never ends up with no description at all. That case is common rather than rare: free Yoast with nothing filled in publishes no description on most pages, and SEOPress publishes none on category or shop pages.
  - Two things are unchanged. Social-share tags can still appear twice, which is tracked separately (#676). And your page title is still filled by the other SEO plugin while it is active, on every page except your shop listing under SEOPress.
...
- **Most AI traffic was recorded as "unknown" (#655).**
  ...
  - One consequence worth knowing: your settings now reach assistants they previously missed, in both directions. An assistant you had blocked is now actually blocked when it identifies itself sloppily, and an unrecognised assistant using one of those spellings is now refused rather than let through, unless you have allowed unknown assistants. Before this change a malformed header slipped past your settings entirely.
...
- **Agents could only ever see the first ten products (#656, #659).**
  ...
  - Sorting had the same problem: the reference said `sort.order`, the code reads `sort.direction`. A missing direction falls back to ascending, so cheapest-first happened to come out right — and asking for most expensive first returned cheapest first, the exact opposite of the request.
  ...
  - The reference now matches the code, and unknown keys inside `filters`, `pagination`, and `sort` come back in the `X-WC-AI-Storefront-Unknown-Params` header instead of vanishing. Browsers can now read that header cross-origin, which they previously could not.
  - Assistants connected over MCP get the same list. MCP has no response headers, so the unrecognized names come back as a warning in the response — named, so the assistant can correct the call rather than just learn something went wrong.
