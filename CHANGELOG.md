...
- **UCP manifest: declare `dev.ucp.shopping.discount` when the capability is active.** The per-response `/checkout-sessions` envelope already advertised the discount capability (from #376), but the discovery surface at `/.well-known/ucp` did not — so agents that consult the manifest upfront to learn what fields to send couldn't discover that `discounts.codes[]` was supported. Now the manifest carries the capability declaration with `version`, `spec`, `schema`, and `extends: dev.ucp.shopping.checkout`, gated on the same `discount_capability_active()` helper that controls the envelope — manifest discovery and runtime envelope stay in lockstep.
...
- **UCP /checkout-sessions: discount-code pass-through.** Closes #376. New optional capability `dev.ucp.shopping.discount` advertised in the checkout-response envelope when active. Agents can supply user-typed promo codes via `discounts.codes[]` on the request body; the first valid code is appended to the `continue_url` as `coupon-code=`. Pass-through scope only — WC's hosted checkout validates and applies the code at the next step; we don't compute `discounts.applied[]` (out of scope for v0.14.2).
  - **Merchant toggle**: new "Allow discount codes through AI agents" checkbox under the Policies tab → Checkout card. Defaults `'yes'`. Setting key: `allow_discount_codes`. **Upgrade note**: existing 0.14.1 installs read the new default on upgrade — the capability becomes active automatically if WC coupons are enabled. Merchants who want UCP discount support OFF while keeping WC coupons on for their storefront should uncheck the new toggle after upgrading.
  - **Truthful discovery**: capability advertisement is gated on BOTH the toggle AND WC core's `wc_coupons_enabled()` — if a merchant disables coupons in WooCommerce → Settings → General, we never advertise the capability regardless of the toggle.
  - **Sanitization**: codes must match `[A-Za-z0-9_-]{1,64}` per entry. Single-code scope for v0.14.2 (first valid wins; remaining entries dropped). Malformed codes are filtered out, not 4xx'd.
