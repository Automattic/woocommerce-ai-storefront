...
### Features
- **Stub `PATCH /checkout-sessions/{id}` returns a structured 405 `unsupported_operation` response.** Background: UCP agents that come from a stateful-session mental model expect to PATCH a session to add items, change quantities, update shipping. Our `/checkout-sessions` model is intentionally stateless — every POST is a fresh computation that returns a one-shot Shareable Checkout redirect, no persisted session to mutate. Without a registered PATCH route, agents got WP REST's generic 404 `rest_no_route` envelope, which they misread as "session expired" or "API down" and may retry destructively. The new stub registers the route exclusively to NOT support the operation: every request returns HTTP 405 with the full UCP checkout-envelope shape, `code=unsupported_operation`, severity `unrecoverable`, and a content message pointing the agent at the POST flow. RFC 7231 `Allow: POST` header included so agents that handle 405 generically can retry without re-reading the manifest. The session-id captured from the URL is echoed back in the envelope's `id` field — preserves the agent's correlation thread even though we hold no state. Posture-preserving: this is an explicit "we will not modify state" surface, not an enabling one. 12 new tests in `UcpCheckoutSessionsPatchTest` pin the contract; the existing `UcpCheckoutPostureTest::test_registered_rest_routes_are_the_exact_posture_set` whitelist was updated with an inline rationale so the route's posture-compatibility is reviewed alongside any future addition.
