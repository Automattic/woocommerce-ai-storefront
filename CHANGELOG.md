...
- **Public MCP (Model Context Protocol) transport for external shopping agents.** A new JSON-RPC 2.0 / Streamable-HTTP endpoint at `POST /wp-json/wc/ucp/v1/mcp` exposes the same shopping capabilities as MCP tools, for agents that engage with MCP but not the UCP REST transport ([#418](https://github.com/Automattic/woocommerce-ai-storefront/issues/418)).
  ...
  - Public but gated by the existing UCP agent allow-list, with identity taken from the MCP `initialize` handshake (`clientInfo.name`) and re-checked on every call. Required-session mode (`Mcp-Session-Id`), `initialize` rate-limiting, and exact-host `Origin` (DNS-rebinding) defense apply. Checkout stays stateless — a `continue_url` handoff, never an inline order. MCP-originated orders resolve the same WC Order Attribution `agent_data` as the REST transport.
