...
- **RTL: Save buttons on the Settings tabs now respect writing direction.** Pre-fix the inline `textAlign: 'right'` on the Discovery and Product Visibility tab footers pinned the Save button to the physical right edge regardless of locale. Under RTL writing systems (Arabic, Hebrew, Persian, Urdu) the Save affordance should sit on the visual end side, which in RTL is the LEFT edge of the screen — `'right'` produced a visually-orphaned button at the wrong end of the row. Replaced with the CSS logical property `textAlign: 'end'`, which tracks `direction` automatically: `right` in LTR, `left` in RTL. Same logical-property migration is also applied to the Policies tab footer in PR #102 so the new footer ships RTL-correct from day one rather than introducing the bug and removing it across two PRs.
