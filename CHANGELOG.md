...
- **JSON-LD: homepage `OnlineBusiness` block now emits `hasMerchantReturnPolicy` at Organization level.** Phase 1 of #337.
  ...
  - No behavior change to existing per-Offer emission. Phase 2 (making per-Offer emission conditional on the per-product final-sale override) is deferred to a separate PR.
