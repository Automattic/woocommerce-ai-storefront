...
### Tests
- Replaced `test_sitemap_directive_reemitted_at_end_of_section` and `test_sitemap_directive_falls_back_to_wp_core_when_no_sitemap_in_input` with a single `test_no_bottom_of_section_sitemap_reemission` regression guard that asserts top-level `Sitemap:` directives appear exactly once in the output (at their original location), not duplicated at the bottom.
