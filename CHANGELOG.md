...
- **UCP: WooCommerce Product Bundles plugin support.** Closes #358 (V2 deterministic-URL scope merged from #361).
  ...
  - **Mixed/multi-bundle cart rejection:** when a bundle is sent alongside other line items (or multiple bundles in one cart), the handler returns `status: incomplete` with one spec-defined `field_required` error per bundle line item (path-attributed via `$.line_items[N]`). No `continue_url`. Agent must split the cart into separate `/checkout-sessions` requests. Replaces the earlier silent-drop + custom advisory message — this version aligns with `error_code.json`'s standard codes and the spec's `requires_buyer_input` severity.
