...
- **Save UX consistency across all four settings tabs.** Pre-fix the Policies tab's Save button was left-aligned (every other tab was right-aligned), the button was always-enabled regardless of unsaved state (clicking it on a clean form re-POSTed unchanged settings), and the "Returns accepted" `ToggleGroupControl` label wrapped to two lines because the WP default sized each segment to ~1/N of the container — which is too narrow for the 16ch label. Three coordinated fixes:
  - **Dirty-aware Save** across Discovery, Product Visibility, Policies, and Endpoints. New `getSavedSettings` + `isDirty` selectors compare the working `settings` draft to a last-known-saved snapshot via `JSON.stringify` round-trip (cheap, insertion-order-stable for our settings shape, order-sensitive for arrays which is the right behavior — every list in our settings has meaningful order). `SET_SETTINGS` writes both copies (initial load + post-save refetch are the "draft equals saved" sync points); `SET_SETTINGS_VALUES` only updates the draft. Type-then-undo correctly returns to clean. Mirrors the WooCommerce Settings + Block Editor `isEditedPostDirty` pattern.
