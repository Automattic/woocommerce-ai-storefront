...
### Tests
- 17 new tests for the unknown-agent gate (`UcpAgentAccessGateTest` and `UpdateSettingsSanitizationTest`): 4 contract pins for the gate (key absent, explicit `'no'`, explicit `'yes'`, raw host in block message), paused-syndication-with-unknown-host precedence, fresh-install default, 7 type-juggled stored values (boolean / int / string-1 / uppercase / whitespace / array — defends the strict-mode `in_array(..., true)` against future loosening), 6 sanitize-layer fallback tests for malformed `update_settings()` payloads, plus a TODO comment locating where REST-schema integration coverage would land.
- 6 new tests in `AttributionTest` covering the lenient attribution gate: production scenario (UCPPlayground host with `utm_medium=referral`), generic known-host canonicalization (openai.com → ChatGPT), unknown-host rejection (defends against random-referrer self-attribution), empty-source skip, strict-gate regression, and the URL-param-overwrites-lenient-stamp interaction when both gates apply.
