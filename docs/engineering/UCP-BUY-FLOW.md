...
When the cart is otherwise valid (in stock, meets minimum, IDs resolve), the `continue_url` shape depends on the cart contents. The plugin emits one of five outcomes; the three that produce a `continue_url` carry the canonical UTM payload (`utm_source`, `utm_medium=referral`, `utm_id=woo_ucp`, plus optional `ai_agent_host_raw` when the agent was identifiable). The `ai_session_id` query param is *agent-supplied* — agents append their own `ai_session_id=chk_…` if they want it captured into order meta; the plugin's server-side stamping helper does not append it:
