...
- **Visible product-page checkout anchor** (`render_product_checkout_links()` on `wp_body_open`, since 0.22.0) — the same deterministic checkout URL the `BuyAction` carries, rendered as visible `<code>` text near the top of `<body>`. Markdown-extraction fetch tools (e.g. claude.ai `web_fetch`) strip `<script>` JSON-LD *and* `<a href>` attributes (keeping only link text), so both the `BuyAction` `urlTemplate` and a plain anchor are unreachable to them; rendering each URL as `<code>` text re-exposes the link where they can read it and hand it to the buyer — the per-product counterpart to the `/llms.txt` body anchor (which moved to `wp_body_open` for the same reason). Ready-made `<code>` URLs (simple, bundle/grouped, concrete variants) carry `utm_source=ucp_unknown` (the no-identity sentinel) to be used as-is, while the construct-kit `<code>` URL *template* keeps `{agent_id}` as the faithful `BuyAction` mirror for agents that substitute their own id. Simple → one URL; variable → the template + a `/products/{handle}.json` link (the lone `<a href>`, gated on the products feed) + concrete per-variant URLs when ≤4 purchasable; bundle/grouped → the permalink URL. Gated on `enabled` + syndication; non-purchasable items skipped.
