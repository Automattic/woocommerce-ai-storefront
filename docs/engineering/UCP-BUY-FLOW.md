...
The agent identity behind `utm_source` resolves in precedence order: UCP-Agent profile → UCP-Agent product → body `meta.source` / declared MCP name → User-Agent-derived → `ucp_unknown` (PR #465). The User-Agent step is the last fallback before `ucp_unknown`: when no explicit identity signal was sent, an answer-agent User-Agent token maps to its brand host (e.g. `ChatGPT-User` → `utm_source=chatgpt.com`, with the raw token stored in `ai_agent_host_raw`). Only the answer agents are mapped — ChatGPT, Claude, and Perplexity user/bot agents — so generic indexers (Bingbot, Googlebot, Applebot) stay `ucp_unknown`. An inferred agent attributes into the same bucket as a declared one; `ai_agent_host_raw` carries the User-Agent token (rather than a hostname) so merchants can tell inferred from declared. This is attribution only — it does not affect access control.
...
- **Enhanced JSON-LD** on product pages with `potentialAction` of type `BuyAction`. The `urlTemplate` is a bare Shareable Checkout URL with no attribution parameters (#574): these JSON-LD URLs are surfaced to human shoppers by search engines, so UTMs were removed to let WooCommerce attribute the click natively. Agent attribution flows through the `UCP-Agent` header on `/checkout-sessions`, not this URL. Archive pages (shop/category/tag/product-search) additionally emit an `ItemList` whose entries are summary-page pointers — each `ListItem` carries the product name and a link to its product page — so an agent reads the full product lineup from one results page and follows through (to the product pages, or to the catalog API / `/products.json` feed) for prices and details.
- **Public GET catalog endpoints** — `GET /catalog/search` and `GET /catalog/lookup` give fetch-based agents that can't POST a JSON body a way to query the catalog directly with query-string params. Same gate, same response envelope as the POST routes. These (plus the archive `ItemList` and `/llms.txt` below) are the reachable surface for markdown-extraction fetch tools that strip `<script>` JSON-LD and `<a href>` attributes — so the `BuyAction` `urlTemplate` reaches them via the catalog API rather than the page DOM.
