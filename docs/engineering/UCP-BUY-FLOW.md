...
- **Visible product-page checkout anchor** (`render_product_checkout_links()` on `wp_footer`, since 0.22.0) — the same deterministic checkout URL the `BuyAction` carries, rendered as a small visible `<a>` in the product-page footer. Markdown-extraction fetch tools (e.g. claude.ai `web_fetch`) strip `<script>` JSON-LD, so the `BuyAction` `urlTemplate` is unreachable to them; this body anchor re-exposes the link where they can read it and hand it to the buyer — the per-product counterpart to the `/llms.txt` body anchor. Clickable links carry `utm_source=ucp_unknown` (the no-identity sentinel — a `{agent_id}` placeholder can't survive `esc_url()` in an `href`), while the non-clickable `<code>` URL *template* keeps `{agent_id}` as the faithful `BuyAction` mirror for agents that substitute their own id. Simple → one link; variable → the template + a `/products/{handle}.json` link + concrete per-variant links when ≤4 purchasable; bundle/grouped → the permalink link. Gated on `enabled` + syndication; non-purchasable items skipped.
