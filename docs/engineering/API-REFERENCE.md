...
- **Non-variable products (simple, bundle, grouped) emit no `options[]`** regardless of attribute names. UCP `product_option.json` characterizes options by example as size, color, or material — variant-selection axes the buyer chooses between. A non-variable WC product has one product ID, one SKU, no buyer selection. Color / Size / Pattern / Material attributes on these products surface as descriptive `metadata.attributes` entries (same path as Origin, Fabric Weight, etc.), with all declared values preserved. Variable products' `has_variations: true` attributes still emit legitimate selectable `options[]`.
