...
| `images[]` | In the **list feeds** (`/products.json` and `/collections/{handle}/products.json`), only the **first valid image** per product (featured if set, else the first valid gallery image), or `[]` when none resolves. The single-product feed `/products/{handle}.json` emits **all** images. Sources, in order and de-duplicated: featured image, gallery, then **images assigned to individual variations**. That last source is a deliberate divergence from a naive port — WooCommerce keeps variation images outside the parent gallery, whereas Shopify guarantees a variant's image is always one of the product's images, so without it `variant_ids` would be empty on every image. Entries with no resolvable URL are dropped. |
