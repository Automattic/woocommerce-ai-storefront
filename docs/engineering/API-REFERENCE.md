...
- **Products with no configured price are not syndicated at all** (#658). WC's Store API money formatter flattens an unset price and a genuine zero to the same `prices.price: "0"`, and UCP `common/types/price.json` documents `amount` as *"Use 0 for free items"* — so emitting the fallback would tell an agent the item is free. `price` is required on `variant.json` and `price_range` on `product.json`, so there is no schema-legal way to say "price unknown"; the product is withheld instead. The discriminator is `price_html`, which WC renders empty only when no price was set, combined with a zero `prices.price` so a store-wide "hide prices" plugin only suppresses products whose lowest price is zero, not the whole catalog. `catalog/search` drops these silently (debug-logged, like the #373 filter above); `catalog/lookup` returns an `item_unpurchasable` message at the requested ID's `$.ids[N]` path — the same code checkout emits for the same condition. **`pagination.total_count` reflects the pre-suppression Store API total and may over-count; `has_next_page` and `cursor` stay accurate.**

**Zero-results hints.** When the response emits no products (`products: []`), the body includes a `hints` object with a step-by-step recovery recipe. `total_count` is a pre-suppression figure (see #658 above) and isn't part of the test, so a non-zero `total_count` alongside an empty `products` array is a state agents can encounter.
