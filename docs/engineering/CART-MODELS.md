...
> **Container-product-type note (post-PR #360 / #362 / #370).** When the cart contains a WooCommerce **bundle**, **grouped**, **variable**, or **variable-subscription** parent, the returned `continue_url` is *not* `/checkout-link/?products=…`. These types carry per-child or per-variation configuration the `?products=` shorthand can't express, so the controller emits one of `/checkout/?add-to-cart=BUNDLE&bundle_*=…`, `/checkout/?add-to-cart=PARENT&quantity[CHILD]=…`, or the parent's PDP permalink. The PDP-permalink fallback applies whenever the deterministic URL isn't available (configurable bundle, configurable grouped, or any variable / variable-subscription parent without a chosen variation). Two cases produce `status: incomplete` with `field_required` errors and **no `continue_url`** — agents split or retry depending on severity:
>
> - **Mixed cart** (bundle/grouped/variable parent alongside other items, or multiple parents of the same kind): one `field_required` error per offending parent line item (JSONPath-attributed via `$.line_items[N]`) with `severity: recoverable` — agent splits the cart into per-parent `/checkout-sessions` calls. The permalink fallback can only redirect to one PDP, which cannot also add a sibling simple-product line item to the cart.
> - **Single configurable parent without permalink** (any of the four types where the deterministic URL helper returned null AND the parent has no usable PDP URL): one `field_required` error with `severity: recoverable` — merchant misconfiguration; the agent can't resolve via splitting and surfaces the message instead.
>
> **`subscription` and `subscription_variation` line items** are accepted via the standard Shareable Checkout `/checkout-link/?products=ID:1` path — the recurring-billing UI on WC's checkout page handles sign-up correctly. Per the PR #367 audit, the pre-#370 belief that subscriptions misroute through Shareable Checkout was empirically contradicted.
>
