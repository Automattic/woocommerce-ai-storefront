...
| `class-wc-ai-storefront-jsonld.php` | Product pages + homepage / shop page + every page + archives | **Per product** (`wp_head`): enhanced Schema.org `Product` markup — typed properties (`color`/`size`/`material`/`pattern`), BuyAction with Shareable Checkout URL, `Offer.checkoutPageURLTemplate`, inventory, attributes, shipping (`OfferShippingDetails` with handlingTime, dispatch `businessDays` and free-shipping rate), per-Offer return policy, and `isRelatedTo`/`isSimilarTo` from cross-sells/upsells. Variable products convert to Schema.org `ProductGroup` with per-variant `hasVariant` entries (with a postmeta-direct override path for misconfigured variations where the parent's "Used for variations" flag is unset). **Homepage / shop page** (`wp_head` priority 5): Schema.org `OnlineBusiness` (an `Organization` subtype) with auto-sourced identity fields — `logo` (custom-logo theme mod → site-icon fallback), `address` (PostalAddress from `WC()->countries->get_base_*` minus `streetAddress`, suppressed for privacy), `contactPoint.email` (two-stage from `woocommerce_email_reply_to_address` → `woocommerce_email_from_address` with noreply guard, never falls back to `admin_email`) — plus `knowsAbout` (top product category names from cached catalog summary) and Org-level `hasMerchantReturnPolicy` when the merchant's return policy is configured. **Every page** (`wp_head` priority 4): a global `WebSite` + `SearchAction` block advertising two search entry points (native WP product search + the public `GET /catalog/search` REST endpoint), so an agent landing on any page can discover how to search; suppressible via the `wc_ai_storefront_jsonld_website` filter for Yoast/Rank Math dedup. **Shop / category / tag / product-search archives** (`wp_head` priority 6): an `ItemList` of `ListItem` summary-page pointers (`position`/`name`/`url`) for the current page — the linked product pages carry the full `Product` node (price, offers, BuyAction, etc.), so Google validates completeness on the product pages rather than the archive; `numberOfItems` is emitted only when visibility mode is `all`; search pages render fresh (not cached) to avoid `wp_options` flooding. The shop gate is `is_shop()` (since 0.23.0), so when the shop archive IS the site front page it emits the product ItemList too — the root then carries product names + links alongside the `OnlineBusiness`/`WebSite` blocks, not just navigational JSON-LD. (Markdown-extraction agents that strip `<script>` JSON-LD reach product data via the archive `ItemList`, the `GET /catalog/*` endpoints, and `/llms.txt` — not a body-rendered checkout anchor, which was removed in 0.23.3 as intrusive to shoppers since it exposed raw checkout URLs and `{variation_id}`/`{agent_id}` placeholders on the page.) See [`JSON-LD-SCHEMA.md`](JSON-LD-SCHEMA.md) for the full field reference and example output. |
| `class-wc-ai-storefront-robots.php` | `/robots.txt` | Allow-lists known AI crawlers (which bots are welcome); each welcomed bot gets a deny-list that blocks cart/checkout/account and leaves everything else crawlable, plus two defensive `Allow:` re-permits for the `/wp-json/` commerce endpoints. |
| `class-wc-ai-storefront-shipping-policy.php` | Homepage `OnlineBusiness` block | Turns WooCommerce shipping zones into Google's Organization-level `hasShippingService` → `ShippingConditions`. Exists because WooCommerce prices shipping by zone AND order value, which product-level `OfferShippingDetails.shippingRate` (one `MonetaryAmount`) cannot express — on a "free over $20, otherwise $20" store no single product-level number is true. A zone with a `min_amount` free method emits a two-band pair (`orderValue.minValue` → rate 0; `orderValue.maxValue` → paid rate). `WC_Shipping_Flat_Rate::$cost` is an EXPRESSION, not a number: only a literal and a bare `[fee percent=…]` are publishable, and anything cart-dependent (`[qty]`, `[cost]`, or a percentage summed with other terms) drops the condition rather than guessing. Only the cheapest method per zone is emitted, since Google applies the lowest matching rate. Zone 0 emits no `shippingDestination`, which is how Google reads "anywhere else". `handlingTime` here is a `ServicePeriod`, deliberately UNLIKE the product block's bare `QuantitativeValue` — Google's examples differ per surface. Returns null (key omitted) when nothing is derivable; an empty `shippingConditions` array would claim the store ships nowhere. |
| `class-wc-ai-storefront-products-feed.php` | `/products.json` **+ `/collections/all/products.json` + `/products/{handle}.json` + `/collections/{handle}/products.json` + `/collections.json`** | **Non-UCP, additive Shopify-compat surface.** Serves the syndicated catalog in Shopify's public `products.json` shape across **five endpoints** AI agents are trained to probe — all behind the same `products_json_enabled` toggle, same edge-cache headers, and same per-product `is_product_syndicated()` gate: (1) **`/products.json`** — the bulk feed (primary catalog-probe path), and (2) **`/collections/all/products.json`** — the secondary alias resolving to the same all-products handler. A WC→Shopify `map_product()` mapper (`title`/`handle`/`body_html`/`published_at`/`created_at`/`updated_at` (RFC 3339 UTC, null when unset)/`vendor`/`product_type`/`tags`/`variants[]`/`images[]`/`options[]`) sourced directly from WC product objects, not the UCP translator (the Shopify shape diverges enough that a dedicated mapper is cleaner). Since #627 the variant and image records are at **full key parity** with a live Shopify feed: variants carry `grams` (integer, converted from the store's configured weight unit), `taxable`, `position`, `product_id`, `created_at`/`updated_at` and `featured_image` alongside the option/price/availability fields; images carry `width`/`height`/`position`/`product_id`/`created_at`/`updated_at`/`alt`/`variant_ids` alongside `id`/`src`. Three behaviours there are load-bearing and easy to undo by accident. (a) **`images[]` also draws from variation-owned attachments**, because WooCommerce keeps variation images outside the parent gallery while Shopify guarantees a variant's image is always one of the product's images — without the union `variant_ids` is empty on every image for a typical store. (b) **Variation image ownership is read in `'edit'` context**; `WC_Product_Variation::get_image_id()` falls back to the parent in `'view'`, which would report every photo-less variation as owning the parent's photo (same bypass idiom as `base_price()`). (c) **A simple product's synthesized variant emits `featured_image: null` and an `options[]` placeholder** (`Title`/`Default Title`) — both measured against Shopify rather than inferred, and the placeholder exists because clients written to Shopify's shape assume the key is present. Paginated (`?limit=` default 30 / max 250, `?page=`). The v2 scoped endpoints reuse that mapper: (3) **`/products/{handle}.json`** — a single product by slug, body `{ "product": { … } }` (**singular `product` OBJECT key**, not the bulk array); 404s when the slug is unknown or resolves only to a hidden/unsyndicated product (never leaks; 404 not cached). (4) **`/collections/{handle}/products.json`** — one `product_cat` by slug, `{ "products": [ … ] }`, paginated; an unknown or empty-after-gate category returns a uniform `200 { "products": [] }`, never 404 (the `(?!all/)` rewrite lookahead keeps `/collections/all/…` on the bulk handler). (5) **`/collections.json`** — the category list `{ "collections": [ … ] }` via a `map_collection()` mapper, listing only categories with ≥1 catalog-visible+syndicated product with `products_count` as that post-gate count; `published_at`/`updated_at` are null (`wp_terms` has no timestamps). Gated on `enabled` + `products_json_enabled` + the per-product rule; 404 otherwise. Edge-cacheable like the other discovery surfaces (`Cache-Control: public, max-age` via `WC_AI_Storefront::discovery_cache_control()`, `Vary: Host`, CORS, OPTIONS→204); origin caches each response under a host-scoped, **versioned-key** transient (four key families — `wc_ai_sf_pjson_`, `wc_ai_sf_prod_`, `wc_ai_sf_coll_`, `wc_ai_sf_colls_`) bumped by the cache invalidator on product/category/settings change. Adds the `wc_ai_storefront_products_feed_collection` filter (mirrors `wc_ai_storefront_products_feed_product`). Does **not** alter the UCP manifest, REST/MCP, llms.txt, or JSON-LD. See [`API-REFERENCE.md`](API-REFERENCE.md#shopify-compatible-products-feed). |
...
The same UCP shopping capabilities re-expressed as MCP tools, for agents whose client speaks Model Context Protocol rather than REST. **Shopper-facing and public** — not to be confused with WooCommerce core's MCP integration, which is admin-side and runs under merchant credentials (see item 9 under "Key design decisions" below).

It adds no commerce logic. Every tool call lands on the same `run_catalog_search()` / `run_catalog_lookup()` / `run_checkout_create()` neutral cores the REST routes use, so filtering, validation, syndication scoping and attribution behave identically on both transports. That shared seam is the point: it is what makes the transport cheap to maintain, and what would make it cheap to replace with WordPress core's MCP adapter if the Abilities API route matures.

Two presentation-layer divergences are deliberate. MCP passes `require_continue_url = true` on checkout, so a 200 carrying no `continue_url` becomes `isError: true` where REST returns it as a success. And MCP appends the unknown-param advisory into `messages[]`, which REST does not do because it would change a documented payload shape.

Module location: `includes/ai-storefront/ucp-mcp/`

| File | Responsibility |
|------|----------------|
| `class-wc-ai-storefront-mcp-server.php` | Minimal Streamable-HTTP JSON-RPC server registered at `POST wc/ucp/v1/mcp`. Runs the gate chain in this order — feature gate (`enabled` **and** `mcp_enabled`, else a bare 404), rate limit, JSON parse, then either the `initialize` branch or a protocol-version check followed by dispatch. `initialize` is handled before the version check, so a handshake is never rejected for the header it hasn't negotiated yet. Identity and allow-list checks are per-method rather than chain-wide: `ping` and `tools/list` run neither, `initialize` runs the allow-list, `tools/call` resolves a caller. Dispatches `initialize`, `notifications/initialized`, `ping`, `tools/list` and `tools/call`. Protocol: advertises `2025-06-18` and accepts `2025-03-26`, assuming the latter when a post-handshake request omits the header (spec back-compat). `initialize` echoes the client's requested version when it is supported rather than always answering with the newest — a strict `2025-03-26` client disconnects otherwise. Rate limiting runs *before* JSON parsing so malformed bodies still spend the caller's budget. `GET` returns 405 with a human-readable hint rather than a blank error, since people do open the URL in a browser. No `Origin`-based 403: this is a public endpoint reached cross-origin by agent web UIs, so access is governed by the allow-list and CORS, not by `Origin`. |
| `class-wc-ai-storefront-mcp-tools.php` | Tool definitions and dispatch. Three tools — `search_catalog`, `lookup_catalog`, `create_checkout` — named per the UCP spec's `mcp.openrpc.json`; the names are a wire contract, not a local choice (see #651). `definitions()` keeps input schemas *close to* the keyword subset every major model's function-calling layer accepts, deliberately omitting `minItems`/`maxItems` and `minimum`/`maximum` because Gemini's function-declaration validator rejects them with a hard 400 that breaks tool registration for the whole session. One `anyOf` is retained on `search_catalog` — it expresses the "query and/or filters" rule no single `required` entry can — and the code flags it as the first thing to drop if a Gemini-specific registration failure ever appears. `call()` routes each name to its core; `core_result_to_mcp()` maps the core's result into an MCP tool result, and on success the `summarize_*()` helpers build the human-readable `content` text alongside `structuredContent`, because some clients surface only the former to the model. On the error path `core_result_to_mcp()` derives that text from `messages[]` itself and never calls a summarizer. |
| `class-wc-ai-storefront-mcp-session.php` | Transient-backed sessions (`wc_ai_mcp_sess_` prefix, 900-second TTL, no sliding renewal). `start()` mints a session at `initialize` and returns the id in the `Mcp-Session-Id` header; `gate_client_name()` applies the merchant's `allowed_crawlers` and `allow_unknown_ucp_agents` settings to the handshake's client name, the MCP-side equivalent of REST's `check_agent_access()`. An expired session is distinguishable from a blocked one: the server maps its 404 to JSON-RPC `-32001` and a 403 to `-32000`, so a client can tell "re-initialize" from "you are not allowed". |

**`tools/list` is ungated and session-free.** Discovery returns tool metadata only — no catalog data — so an agent can learn the surface before handshaking. Everything that touches products goes through `tools/call`, which requires an *identity* but not a session: a valid `Mcp-Session-Id` supplies a vetted one, and without it the caller is anonymous and admitted only when the merchant has set `allow_unknown_ucp_agents`.

**No server-initiated stream.** There is no SSE channel, so the store can never speak first. Every interaction is agent-initiated, which rules out push notifications ("the item you wanted is back in stock") on this transport.

**Advisory parity, on search.** Unrecognized request keys on `POST /catalog/search` surface as an `X-WC-AI-Storefront-Unknown-Params` header. MCP has no response headers, so `search_catalog` surfaces the same detection as a `messages[]` warning naming the keys (see #656). Neither transport reports unknown keys on lookup or checkout.
...
| File | Purpose |
|------|---------|
| `class-wc-ai-storefront-store-api-rate-limiter.php` | Two-layer rate limiting for AI bot traffic. (1) **Outer layer:** `check_outer_rate_limit()` is called by `check_agent_access()` on the REST path, and directly from `WC_AI_Storefront_MCP_Server::handle()` on the MCP path (which has no `check_agent_access()` in its chain) and counts exactly one slot per logical outer UCP request (e.g. one `/catalog/lookup` with 50 IDs = 1 slot). Uses a per-fingerprint WP transient with a 60-second sliding window. (2) **Inner layer suppression:** `configure_rate_limits()` returns `enabled: false` when `WC_AI_Storefront_UCP_Store_API_Filter::is_in_ucp_dispatch()` is true, so WC's built-in per-Store-API-call counter is disabled for inner `rest_do_request()` dispatches. Fingerprints AI bots by user-agent via `woocommerce_store_api_rate_limit_id`. Regular customer traffic is unaffected; direct `/wc/store/v1/` requests from AI bot UAs outside the UCP bracket remain subject to WC's default counter. The merchant's `rate_limit_rpm` setting reflects outer-request semantics. |

### Attribute seeding

| File | Purpose |
|------|---------|
| `class-wc-ai-storefront-attribute-seeder.php` | Creates the six global product attributes Google expects on apparel — `pa_gender`, `pa_age_group`, `pa_color`, `pa_size`, `pa_material`, `pa_pattern` — because a fresh WooCommerce store ships with none and WooCommerce itself takes no position on them. Runs from the version-mismatch branch in `WC_AI_Storefront::register_rewrite_rules()`, so it fires on activation and on every upgrade, and is deferred to `init` (activation runs *after* `init`, so an unconditional `add_action( 'init', … )` there would register a callback that never fires while the one-shot branch was consumed anyway; `schedule_attribute_seeding()` branches on `did_action( 'init' )` for exactly this reason). Idempotent and decided **per attribute** — an existing taxonomy is skipped entirely, terms included, because its terms may be variation axes on live products. Gender and Age group carry Google's exhaustive value lists; Color, Size, Material and Pattern carry a small starting set merchants extend, since Google treats those as free text matched to the merchant's own page. `wc_create_attribute()` does not register the taxonomy in the same request, so `register_taxonomy()` is called before term insertion, mirroring WooCommerce's own CSV importer. Opt out with the `wc_ai_storefront_seed_attributes` filter. Feeds the typed `Product.audience` emission in the JSON-LD generator. See [`DATA-MODEL.md`](DATA-MODEL.md#taxonomies-and-terms). Since 0.35.0. |
...
│       ├── class-wc-ai-storefront-content-meta-tags.php   # Same, for posts/pages when nothing else emits (#680)
│       ├── class-wc-ai-storefront-meta-text.php          # Shared cleaning, truncation and locale helpers
│       ├── class-wc-ai-storefront-meta-image.php         # Shared image resolution + the printability rule
...
│       └── ucp-mcp/
│           ├── class-wc-ai-storefront-mcp-server.php
│           ├── class-wc-ai-storefront-mcp-tools.php
│           └── class-wc-ai-storefront-mcp-session.php
│
├── client/
...
9. **No MCP *abilities* registered with WooCommerce core yet, but the path is open.** (This is about admin-side ability registration only. The plugin's own shopper-facing MCP transport is a separate thing and is documented under "UCP MCP transport" above.) WooCommerce **core** now ships native MCP support starting in WC 10.3.0 (developer preview behind the `mcp_integration` feature flag, see [WC's MCP docs](https://github.com/woocommerce/woocommerce/blob/trunk/docs/features/mcp/README.md)). The integration lives in `plugins/woocommerce/src/Internal/MCP/` inside the core plugin (no separate extension); third-party plugins can register MCP-exposed abilities via the WordPress Abilities API (`wp_register_ability()`) and have them automatically surfaced through the shared WordPress MCP adapter, with no separate auth/transport/routing layer needed. Note the version floor: AI Storefront's minimum WC requirement is currently 9.9, so any MCP-registering code paths would have to feature-detect (`function_exists('wp_register_ability')` or a `WC_VERSION` check) to avoid breakage on WC 9.9–10.2. **The MCP and UCP audiences are orthogonal, and this item concerns the admin-side one only.** The distinction is the audience, not the wire format: **WC core's MCP integration** is **admin-side** (the merchant's own AI assistants, like Claude or ChatGPT, acting on the store from inside the admin under merchant credentials), while **UCP — and our MCP re-expression of it** — is **shopper-side** (external buyers' shopping agents discovering the catalog over the public web, no merchant auth involved). Confusing the two is a meaningful failure mode: core's integration exposes admin operations gated by `permission_callback` and a REST API key, while UCP — over REST or over our MCP transport — exposes only what the merchant configured as syndicated to anonymous external traffic. Candidate AI Storefront abilities for future MCP registration are therefore all *merchant-tooling* abilities: catalog-taxonomy audit (read WC categories + score against canonical product taxonomy), JSON-LD validation (run the plugin's own checks against a single product), syndication-state queries (what would `/llms.txt` say about product N?), and attribution-stats fetches. Each would be a small `wp_register_ability()` call with appropriate `permission_callback` and `mcp.public` metadata. None of these would change UCP behavior or expose anything shopper-facing.
