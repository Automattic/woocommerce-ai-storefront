...
| `class-wc-ai-storefront-products-feed.php` | `/products.json` **+ `/collections/all/products.json`** | **Non-UCP, additive Shopify-compat surface.** Serves the syndicated catalog in Shopify's public `products.json` shape at the endpoints AI agents are trained to probe (`/products.json` is the primary catalog-probe path we observe; `/collections/all/products.json` is the secondary, aliased to the same all-products handler). A WC→Shopify `map_product()` mapper (`title`/`handle`/`body_html`/`vendor`/`product_type`/`tags`/`variants[option1/2/3,price,compare_at_price,available]`/`images`/`options`) sourced directly from WC product objects, not the UCP translator (the Shopify shape diverges enough that a dedicated mapper is cleaner). Paginated (`?limit=` default 30 / max 250, `?page=`). Gated on `enabled` + `products_json_enabled` + the per-product `is_product_syndicated()` rule; 404 otherwise. Edge-cacheable like the other discovery surfaces (`Cache-Control: public, max-age` via `WC_AI_Storefront::discovery_cache_control()`, `Vary: Host`, CORS, OPTIONS→204); origin caches each `(limit, page)` page under a host-scoped, **versioned-key** transient bumped by the cache invalidator on product/settings change. Does **not** alter the UCP manifest, REST/MCP, llms.txt, or JSON-LD. See [`API-REFERENCE.md`](API-REFERENCE.md#shopify-compatible-products-feed). |
