...
It adds no commerce logic. Every tool call lands on the same `run_catalog_search()` / `run_catalog_lookup()` / `run_checkout_create()` neutral cores the REST routes use, so filtering, validation, syndication scoping, error envelopes, and attribution behave identically on both transports. That shared seam is deliberate: it is what makes the transport cheap to maintain, and what would make it cheap to replace with WordPress core's MCP adapter if the Abilities API route matures.

Module location: `includes/ai-storefront/ucp-mcp/`

| File | Responsibility |
|------|----------------|
| `class-wc-ai-storefront-mcp-server.php` | Minimal Streamable-HTTP JSON-RPC server registered at `POST wc/ucp/v1/mcp`. Runs the gate chain — feature gate (`enabled` **and** `mcp_enabled`, else a bare 404 so a disabled transport doesn't advertise its own existence), protocol-version check, session check, agent allow-list, rate limit — then dispatches `initialize`, `notifications/initialized`, `ping`, `tools/list` and `tools/call`. Protocol: advertises `2025-06-18`, accepts `2025-03-26`, and assumes the latter when a post-handshake request omits the header (spec back-compat). Rate limiting runs *before* JSON parsing so malformed bodies still spend the caller's budget. `GET` returns 405 with a human-readable hint rather than a blank error, since people do open the URL in a browser. No `Origin`-based 403: this is a public endpoint reached cross-origin by agent web UIs, so access is governed by the allow-list and CORS, not by `Origin`. |
| `class-wc-ai-storefront-mcp-tools.php` | Tool definitions and dispatch. Three tools — `search_catalog`, `lookup_catalog`, `create_checkout` — named per the UCP spec's `mcp.openrpc.json`; the names are a wire contract, not a local choice (see #651). `definitions()` keeps input schemas inside the keyword subset every major model's function-calling layer accepts, deliberately omitting `minItems`/`maximum` and friends because some validators reject them hard enough to break tool registration for a whole session. `call()` routes each name to its core; `core_result_to_mcp()` maps the core's result into an MCP tool result, and the `summarize_*()` helpers build the human-readable `content` text alongside `structuredContent`, because some clients surface only the former to the model. |
| `class-wc-ai-storefront-mcp-session.php` | Transient-backed sessions (`wc_ai_mcp_sess_` prefix, 900-second TTL, no sliding renewal). `start()` mints a session at `initialize` and returns the id in the `Mcp-Session-Id` header; `gate_client_name()` applies the merchant's `allowed_crawlers` and `allow_unknown_ucp_agents` settings to the handshake's client name, the MCP-side equivalent of REST's `check_agent_access()`. An expired session returns a distinct error code so clients can tell "re-initialize" from "you are blocked". |

**`tools/list` is ungated and session-free.** Discovery returns tool metadata only — no catalog data — so an agent can learn the surface before handshaking. Everything that touches products goes through `tools/call`, which requires a session and passes the allow-list.
...
**Advisory parity.** Unrecognized request keys surface as an `X-WC-AI-Storefront-Unknown-Params` header on REST. MCP has no response headers, so the same detection surfaces as a `messages[]` warning naming the keys (see #656).
