...
| `class-wc-ai-storefront-ucp.php` | `/.well-known/ucp` + `<head>` | JSON manifest declaring implemented capabilities (catalog, checkout), pointing at the UCP REST adapter, advertising empty `payment_handlers` for the redirect-only posture. Also injects `<link rel="ucp-agent" href="/.well-known/ucp">` via `wp_head` so head-scraping agents discover the manifest without needing to probe the well-known path. |
