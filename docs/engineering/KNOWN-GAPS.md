...
- **User-Agent attribution narrows cohort 1 for answer-agents (#465).** When no explicit signal resolves the agent, the resolver now derives attribution from the User-Agent header as the last step before `ucp_unknown` — REST `resolve_agent_host()` ("Path 3.5") and the empty-name branch of MCP `resolve_agent_data_from_name()`. The new `UA_AGENT_HOSTS` map on [`class-wc-ai-storefront-ucp-agent-header.php`](../../includes/ai-storefront/ucp-rest/class-wc-ai-storefront-ucp-agent-header.php) covers **answer-agents only**: `ChatGPT-User`/`GPTBot`/`OAI-SearchBot` → `chatgpt.com`, `Claude-User`/`ClaudeBot`/`Claude-SearchBot` → `claude.ai`, `Perplexity-User`/`PerplexityBot` → `perplexity.ai`. A UA-identified answer-agent now attributes to a real brand instead of `ucp_unknown`, and because its name is no longer the fallback sentinel it also crosses the `FALLBACK_SOURCE` guard — so the same request becomes visible in Discovery stats, not just Recent AI Orders. The raw UA token (e.g. `ChatGPT-User`) is stored in `_wc_ai_storefront_agent_host_raw`, so merchants can tell an inferred attribution from a declared one. **This narrows the gap, it does not close it.** It does not help the *Chromium-based agentic browser* cohort this section is about — Operator/Atlas/Comet drive a real browser and carry no AI token in their UA, so there is nothing for `classify_user_agent()` to match. Generic indexers (Bingbot, Googlebot, Applebot) are deliberately left out of `UA_AGENT_HOSTS` and still resolve to `ucp_unknown`. Attribution only — `check_agent_access` is untouched, so a spoofed UA earns attribution credit but zero access.
...
1. **Track new vendor UA suffixes as they appear.** When agentic products start carrying identifying tokens in their UA (e.g. `Comet/1.0`, `Atlas/1.0`, `Manus/1.0`), we add them to `AI_CRAWLERS` and the brand map. This catches up over time but cannot get ahead of vendors that choose not to identify. (#465 already mapped the *answer-agent fetch/crawl* tokens — `ChatGPT-User`, `ClaudeBot`, `PerplexityBot`, etc. — into `UA_AGENT_HOSTS` for attribution; the still-open part is the *agentic-browser* suffixes above, which those products mostly don't set.)
