...
IndexNow is a protocol that lets you push URLs to search engines the moment your catalog changes, instead of waiting for those engines to crawl on their own schedule. When a product, category, brand, or shop page changes, the plugin batches the affected URLs (plus your discovery surfaces: homepage, `/shop/`, `/llms.txt`) and submits them in the background via WP-Cron. IndexNow accepts up to 10,000 URLs per request, so a large catalog goes out as several requests a minute apart rather than being cut short. Assigning categories, tags or attributes to a product counts as a change, even when the product itself isn't re-saved, which is what happens during an import or a bulk edit. The engines that consume IndexNow are **Microsoft Bing, Yandex, Seznam, Naver, and Yep**. **Google does not use IndexNow.** It relies on sitemaps and its own crawl schedule, so this feature complements your existing structured data and sitemap rather than replacing them.
...
Google disapproves adult products that aren't labelled as such. The **Adult content** checkbox in the product editor's Inventory tab adds that label.
...
### I see two social tags in my page source

You have both AI Storefront and a dedicated SEO plugin (Yoast, Rank Math, SEOPress, AIOSEO) active, and both emit metadata. Two of those tags stay single. The page `<title>` is one, and it is usually the other SEO plugin that fills it. The meta description is the other, because AI Storefront steps aside whenever the other plugin is supplying one. What can still appear twice is the Open Graph and Twitter social tags. Search engines pick one, so it isn't harmful. To make it clean, deactivate one of the two for your commerce pages, and see [section 4c](#4c-search-engine-and-social-metadata) for the hand-off checklist.

If you see two *meta descriptions*, that is worth reporting: it should no longer happen on product, category, or shop pages.
