...
| `class-wc-ai-storefront-ucp-store-api-filter.php` | Hooks `pre_get_posts` (gated by an internal UCP-dispatch depth counter, plus `post_type === 'product'`) to enforce the plugin's `product_selection_mode` setting on UCP-controller-initiated Store API product queries. Front-end Cart, block-theme Checkout, themes, and third-party Store API consumers run outside the UCP dispatch and are untouched — the scope only kicks in when the UCP REST controller calls `rest_do_request()` after `enter_ucp_dispatch()`. Intersects with incoming `post__in` and merges with incoming `tax_query` (outer AND) rather than overriding, so the merchant's allow-list can't be bypassed AND the caller's existing filters stay in effect. (Pre-0.1.15 the file was registered against `woocommerce_store_api_product_collection_query_args`, a hook name that does not exist in WC core; the callback never ran in production until the registration target was corrected.) |
