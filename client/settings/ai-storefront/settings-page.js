...
const AISyndicationSettings = () => {
  ...
  const isLoading = useSelect((select) => {
    ...
    return isResolving("getSettings") || !hasFinishedResolution("getSettings");
  }, []);
  ...
  if (isLoading) {
    return (
      <div style={{ textAlign: "center", padding: "40px" }}>
        <Spinner />
        <p>{__("Loading settings…", "woocommerce-ai-storefront")}</p>
      </div>
    );
  }
        ...
        <div>
            ...
            style={{
              background:
                isSaving || ctaHovered
                  ...
                  : colors.wooPurple50,
              border: `1px solid ${
                isSaving || ctaHovered ? colors.wooPurple70 : colors.wooPurple50
              }`,
              color: "#fff",
              padding: "8px 16px",
              borderRadius: radii.sm,
          ...
          >
            {isSaving
              ...
              : __("Enable AI Storefront", "woocommerce-ai-storefront")}
          </button>
...
const PostEnableView = ({ settings, onChange, onSave, isSaving }) => {
  const stats = useSelect((select) => select(STORE_NAME).getStats(), []);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      apiFetch({
        ...
        .then((response) => {
          if (!cancelled && response && typeof response.count === "number") {
  ...
  if (productCount === "error") {
    productCountDisplay = __("Couldn\u2019t load", "woocommerce-ai-storefront");
  ...
  return (
    <div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  alignItems: "center",
                  border: `1px solid ${
                    isActive ? colors.accent : colors.borderSubtle
                  }`,
                  borderRadius: radii.sm,
      ...
      >
        <StatCard
          label={__("Products exposed", "woocommerce-ai-storefront")}
        ...
        <StatCard
          ...
          value={stats ? formatMoney(stats, stats.ai_revenue) : "\u2014"}
        />
        <StatCard
        ...
        {/* Top agent's share of AI orders — promoted from a
				     ...
				     24px green numerics. */}
        <StatCard
          label={__("Top agent share", "woocommerce-ai-storefront")}
          value={
      ...
      >
        <div>
          <strong style={{ color: colors.textPrimary }}>
            {__("Disable AI Storefront", "woocommerce-ai-storefront")}
          </strong>
          <p
            style={{
              margin: "4px 0 0",
...
const OverviewTab = ({ settings, onChange, onSave, isSaving }) => {
  ...
  return (
    <PreEnableView onChange={onChange} onSave={onSave} isSaving={isSaving} />
  );
};
