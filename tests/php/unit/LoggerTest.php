<?php
/**
 * Tests for WC_AI_Storefront_Logger.
 *
 * The logger is the observability layer — off by default, enabled
 * per-request via the `wc_ai_storefront_debug` filter. Tests cover
...
class LoggerTest extends \PHPUnit\Framework\TestCase {
	...
	public function test_is_enabled_returns_false_by_default(): void {
		...
		Filters\expectApplied( 'wc_ai_storefront_debug' )
			->once()
			->with( false )
			->andReturn( false );

		$this->assertFalse( WC_AI_Storefront_Logger::is_enabled() );
	}
	...
	public function test_is_enabled_returns_true_when_filter_returns_true(): void {
		...
		$this->assertTrue( WC_AI_Storefront_Logger::is_enabled() );
	}
	...
	public function test_is_enabled_caches_result_for_lifetime_of_request(): void {
		...
		WC_AI_Storefront_Logger::is_enabled();
		WC_AI_Storefront_Logger::is_enabled();
		WC_AI_Storefront_Logger::is_enabled();
	}
	...
	public function test_debug_is_a_noop_when_logging_disabled(): void {
		Filters\expectApplied( 'wc_ai_storefront_debug' )->andReturn( false );
		...
		WC_AI_Storefront_Logger::debug( 'this message should never surface' );
	...
	public function test_debug_formats_sprintf_arguments(): void {
		...
		Filters\expectApplied( 'wc_ai_storefront_debug' )->andReturn( true );
	...
	public function test_debug_passes_message_through_unchanged_with_no_args(): void {
		...
		try {
			...
			WC_AI_Storefront_Logger::debug( 'status: 100% complete' );
