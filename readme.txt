...
= 0.38.1 - 2026-08-18 =
**Fixed**
* Upgrading could create a duplicate Condition attribute, so two copies appeared under Products then Attributes. Upgrading to this version removes the extra copy and keeps your values. Don't delete a duplicate yourself from Products then Attributes: WooCommerce removes every value in the attribute when you do that, which empties the copy you keep.
...
= 0.34.1 - 2026-07-21 =
**Fixed**
* AI shopping assistants that identify themselves in a "Product/Version" style header (for example Claude, sent as `Claude/4.6 (Anthropic)`) are now attributed to the correct brand in your order reports instead of an "unknown" bucket.

= 0.34.0 - 2026-07-10 =
**New**
* When you schedule a sale in WooCommerce (a sale price with a start and/or end date), those dates are now published in your product's structured data. Search engines and AI answer engines can see exactly when the sale price is active, so a limited-time promotion is represented accurately when they cite your products. Variable products publish each variation's own sale window, and products that aren't on sale are unaffected.
...
= 0.32.0 - 2026-07-03 =
**Changed**
* Checkout and search links in your product structured data no longer carry tracking parameters. When a search engine shows one of these links to a shopper, WooCommerce now records the real source of the visit (for example "Organic: Google") instead of mislabeling the sale as AI-referred. Orders that came through a checkout link are flagged from WooCommerce's own visit data for future reporting.
...
= 0.30.0 - 2026-07-02 =
**New**
* Yahoo's Slurp crawler is now recognized as a regional (Asia) crawler you can opt into on the Discovery tab.

= 0.29.2 - 2026-06-26 =
**Fixed**
* After you update the plugin, your product-list structured data refreshes right away instead of possibly serving the previous version's output for up to an hour.

= 0.29.1 - 2026-06-26 =
**Fixed**
* Product listings on your shop, category, and search pages now show every product the page displays. On some themes the list was capped below the number actually shown (e.g. 10 listed vs 16 rendered); it now matches the page exactly.
...
= 0.28.3 - 2026-06-25 =
**Fixed**
* Product SEO titles no longer repeat the brand when it matches your store name or already appears in the product name (so an in-house label reads "Camp Shirt – Saltwarp", not "Camp Shirt | Saltwarp – Saltwarp"). The brand still appears when it adds information.

= 0.28.2 - 2026-06-23 =
**Improved**
* IndexNow now generates its verification key automatically when you enable it, so the card is ready to use with no manual "Generate" step. You can still regenerate the key manually.

= 0.28.1 - 2026-06-23 =
**Fixed**
* IndexNow submissions now validate correctly. The verification key file was being redirected to a trailing-slash URL that search engines could not validate, so submitted URLs were silently dropped; it now serves directly.
...
= 0.23.7 - 2026-06-20 =
**Fixed**
* Your shop and homepage product lists now include each product's star rating in the structured data that AI assistants and Google read, when the product has reviews, matching your individual product pages. Products without reviews are unaffected, and ratings are never made up.

= 0.23.6 - 2026-06-20 =
**Fixed**
* Your shop and homepage product lists now include each product's brand (and its GTIN, if you've set one) in the structured data that AI assistants and Google read, matching what's already on your individual product pages. This clears a Google "missing brand/GTIN" recommendation on those lists. Products without a brand or GTIN are unaffected.

= 0.23.5 - 2026-06-20 =
**Fixed**
* Your product prices now always appear in the structured data Google reads, fixing a critical "Missing field price" error in Google's Rich Results. Some recent WooCommerce versions placed the price where Google's merchant listings did not pick it up; prices now show correctly in Search and to AI assistants.
* Virtual and downloadable products no longer advertise shipping details. Only products that actually ship now include shipping and handling-time information in their structured data, matching what AI assistants and Google expect.

= 0.23.4 - 2026-06-19 =
**Fixed**
* Fixed a Google Search structured-data error ("Unnamed item") on your shop and homepage product lists. The product carousel now uses the format Google expects, so AI assistants and Search read your product names, prices, and images correctly.
...
= 0.23.2 - 2026-06-18 =
**Added**
* A "Settings" link now appears on the plugin's row on the Plugins screen, so you can jump straight to the AI Storefront settings instead of finding them under the WooCommerce menu.
...
= 0.22.1 - 2026-06-18 =
**Fixed**
* The product feed that AI shopping assistants read now always shows prices in your store's base currency, instead of sometimes showing a converted currency based on where the request came from. Only affects stores using multi-currency; the price assistants quote now matches your store's base price.
...
* /llms.txt (and its /agents.md mirror) now spells out the agent flow (discover, search, look up, check out, hand the buyer a continue_url to pay) and a read-only browsing section, so AI assistants know exactly how to shop your store.
